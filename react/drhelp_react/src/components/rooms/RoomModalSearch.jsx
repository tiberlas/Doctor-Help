...
class RoomModalSearch extends Component {
    ...
    handleChangeTime = (time) => {
        this.setState({time: time}, () => { this.handleValidation()})
    }
    ...
    handleSearch = (event) => {
        ...
        this.props.handleSearch(this.state.name, this.state.number, this.state.procedureTypeId, this.state.date, this.state.time)
    }
        ...
        return ( 
            ...
            show={this.props.show}>
                ...
                <Modal.Header closeButton onClick={this.props.onHide}>
                    <Modal.Title id="contained-modal-title-vcenter">Search for room</Modal.Title>
                </Modal.Header>
                <form onSubmit={this.handleSearch}>
                    <Modal.Body>

                    <div class='form-group'>
                        <label class="form-control-label" for="name">Room's name:</label>
                        <input type='text' name='name' id='name' class='form-control' value={this.state.name} onChange={this.handlerChange} />
                    </div>

                    <div className={`form-group ${this.state.errorNumber? 'has-danger': ''}`}>
                        <label class="form-control-label" for="number">Room's number:</label>
                        <input type='number' min='1' name='number' id='number' className={`form-control ${this.state.errorNumber? 'is-invalid': 'is-valid'}`} value={this.state.number} onChange={this.handlerChange} />
                    ...
                    <div className={`form-group ${this.state.errorDate? 'has-danger': ''}`}>
                        ...
                        <input type='date' name='date' id='date' className={`form-control ${this.state.errorDate? 'is-invalid': 'is-valid'}`} value={this.state.date} onChange={this.handlerChange} />
                        {(this.state.errorDate) && <div class="invalid-feedback"> Must enter date if time is defined. </div>}
                    </div>
                    ...
                    <Modal.Footer>
                        <div class="form-group row" >
                            <div class='col-md text-left'>
                                <input type="submit" value="Search" class='btn btn-success' data-dismiss="modal" disabled={this.state.errorNumber || this.state.errorDate || this.state.errorTime}></input>
                            </div>
